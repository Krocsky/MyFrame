...
$('body').delegate('.triggerChooseDate','click',function(){
	WdatePicker();
})
$('body').delegate('.triggerChooseDateTime','click',function(){
	...
	var dateFilter = avalon.filters;
	var oneDay = 1000*60*60*24;
	var toDayTime = new Date().getTime();//今天的毫秒
	var toDay  = dateFilter.date(toDayTime, "yyyy-MM-dd");
	var yesterday = dateFilter.date(new Date().getTime()-oneDay, "yyyy-MM-dd");
	var nextWeek = getNextWeek(year,month+1,day).splice(',')
	var XGJDateCollection = {
		'不限':['',''],
		'今天':[toDay,toDay],
		'昨天':[yesterday,yesterday],
		'明天':[
				dateFilter.date(( toDayTime+oneDay), "yyyy-MM-dd"),
				dateFilter.date(( toDayTime+oneDay), "yyyy-MM-dd")
			  ],
		'本周':[
				dateFilter.date(new Date(year, month, day - w + 1), "yyyy-MM-dd"),
				dateFilter.date(new Date(year, month, day + 6 - w + 1), "yyyy-MM-dd")
			  ],
		'本月':[
				dateFilter.date(new Date(year, month, 1), "yyyy-MM-dd"),
				dateFilter.date(new Date(new Date(year, month + 1, 1) - oneDay), "yyyy-MM-dd")
			  ],
		'上月':[
				dateFilter.date(new Date(year, month - 1, 1), "yyyy-MM-dd"),
				dateFilter.date(new Date(new Date(year, month, 1) - oneDay), "yyyy-MM-dd")
		      ],
		'下周':[
				nextWeek[0],
				nextWeek[1]
		...
		'最近7天':[
				dateFilter.date(( toDayTime- oneDay*6), "yyyy-MM-dd"),
				dateFilter.date(toDayTime, "yyyy-MM-dd")
				],
		'最近30天':[
				dateFilter.date(( toDayTime- oneDay*29), "yyyy-MM-dd"),
				toDay
				 ],
		'YEARS':[2010,2011,2012,2013,2014,2015,2016,year+1]
	};
	//获取下一周的时间
	function getNextWeek(year,month,day)  {
         ...
         var monday=new Date(today.valueOf()-(week-8)*oneDay);
         var sunday=new Date(monday.valueOf()+6*oneDay);
         return [
		         monday.getFullYear()+ "-"+((monday.getMonth()+1).toString().length==1?("0"+(monday.getMonth()+1)):(monday.getMonth()+1))+"-"+(monday.getDate().toString().length==1?"0"+monday.getDate():monday.getDate()),
		         sunday.getFullYear()+"-"+((sunday.getMonth()+1).toString().length==1?("0"+(sunday.getMonth()+1)):(sunday.getMonth()+1))+"-"+(sunday.getDate().toString().length==1?"0"+sunday.getDate():sunday.getDate())
         ]
     }

Window.XGJDateCollection
