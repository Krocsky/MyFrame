...
namespace Common.Utilities
{
    ...
    public static class StringHelper
    {
        ...
        #region 生成上传文件的OriginName

        public static string GenerateOriginName(string fileName, DateTime now)
        {
            string originName = string.Empty;
            originName = now.ToString("yyyyMMddHHmmss") + new Random().Next(1000, 10000).ToString() + Path.GetExtension(fileName);
            return originName;
        }
        ...
        {
            return DateTime.Now.ToString("yyMMddHHmm") + new Random().Next(10, 100).ToString() + DateTime.Now.Ticks.ToString().Substring(DateTime.Now.Ticks.ToString().Length - 3, 3);
        }
